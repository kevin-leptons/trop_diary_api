...
describe('schema://atom/log_level', () => {
    ...
    it('verify(info)', async () => {
        schema_service.verify(schema, 0)
    })
    ...
    it('verify(error)', async () => {
        schema_service.verify(schema, 2)
    })
    ...
    it('verify(fatal)', async () => {
        schema_service.verify(schema, 4)
    })
    ...
    it('verify(5) => error', async () => {
        assert.throws(() => {
            schema_service.verify(schema, 5)
        }, InvalidData)
    })

    it('verify(string) => error', async () => {
        assert.throws(() => {
            schema_service.verify(schema, 'info')
        }, InvalidData)
    })
})
