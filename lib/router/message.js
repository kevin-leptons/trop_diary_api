...
get('/', async (req, res) => {
    ...
    req.sv.dflow.verify('//trop/front/get_message#/req/query', req.query)
...
get('/item/:id', async (req, res) => {
    ...
    req.sv.dflow.verify('//trop/front/get_message_item#/req/param', req.params)

    let id = uuid.from(req.params.id)
    let item = await req.sv.message_store.find(id)
    if (!item) {
        throw new Http404('No such message ID')
    } else {
        item._id = uuid.from(item._id).toString()
        res.json(item)
    }
...
post('/', async (req, res) => {
    ...
    req.sv.dflow.verify('//trop/front/post_message#/req/body', req.body)
