...
class Auth {
    ...
    */
    async verify(req, roles) {
        ...
        if (!auth) {
            throw new Http401('Invalid Authentication Header')
        }
        ...
        switch (auth.type) {
            ...
            case 'key':
                ...
                break
            default:
                throw new Http401(`Invalid Authentication Type "${auth.type}"`)
        }
        ...
        try {
            ...
            if (roles.indexOf(token.role) < 0) {
                throw new Http403(`Require ${roles}, you are ${token.role}`)
            }
            return token
        } catch (e) {
            throw new Http401('Invalid token', e)
        }
    }
    ...
    async _create_token_password(username, password) {
        ...
        if (!account) {
            throw new Http401(`Invalid email or password`)
        }
    ...
    async _create_token_refresh(refresh_token) {
        ...
        if (!old) {
            throw new Http401('No such refresh token')
        }

        let account = await this._account_store.find(old.account_id)
        if (!account) {
            throw new Http401(`No such account ${old.account_id}`)
        }
