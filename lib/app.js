...
class App {
    ...
    */
    ...
    async start() {
        await this._init()
        let host = this._conf.host
        let port = this._conf.port
    ...
    _read_conf(conf) {
        ...
        let private_key = fconf.private_key ?
            path.join(conf_dir, fconf.private_key) : null
    ...
    async _init() {
        ...
        this._app.use(middleware.content_type())
        this._app.use(body_parser.raw({
