# /etc/trop_diary_api/private.pem

```text
-----BEGIN RSA PRIVATE KEY-----

zdK+x/Ipmx50F71difoKTz/abxBniepPcYsnXtX8Ffc4pn9VEtkA
-----END RSA PRIVATE KEY-----
```

* Checkout [Private Enhanced Mail](https://en.wikipedia.org/wiki/Privacy-Enhanced_Mail)
  for more information about file format
* Run `bash openssl genrsa -out /etc/trop_diary_api/private.pem 2048` to
  generate new one
